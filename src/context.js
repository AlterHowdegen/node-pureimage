...
class Context {
    ...
    // Source Destination
    drawImageInterpolated(bitmap, sx,sy,sw,sh, dx, dy, dw, dh) {
        // Get scale factors
        var scaleFactorX = Math.floor(sw/dw);
        var scaleFactorY = Math.floor(sh/dh);
        for(var i=0; i<dw; i++) {
            // This needs to be a range of pixels
            var ssxRange = [];
            // Add x coordinates to ssxRange
            for(var k = 0; k < scaleFactorX; k++){
              ssxRange.push(i * scaleFactorX + k);
            }
            ...
            for(var j=0; j<dh; j++) {
                ...
                var ssyRange = [];
                // Add x coordinates to ssxRange
                for(var l = 0; l < scaleFactorY; l++){
                  ssyRange.push(j * scaleFactorY + l);
                }

                // Get rgba values of all pixels in source image range

                var rgbaRange = [];
                for(var m = 0; m < ssxRange.length; m++){
                  for(var n = 0; n < ssyRange.length; n++){
                    // Sum up the rgba values
                    rgbaRange.push(bitmap.getPixelRGBA_separate_i(ssxRange[m],ssyRange[n]));
                  }
                }

                // Get average or mean here
                ...
                for( var o = 0; o < rgbaRange.length; o++ ){
                    sumR += rgbaRange[o][0];
                    sumG += rgbaRange[o][1];
                    sumB += rgbaRange[o][2];
                    sumA += rgbaRange[o][3];
                ...
                var avgR = Math.floor(sumR/rgbaRange.length);
                var avgG = Math.floor(sumG/rgbaRange.length);
                var avgB = Math.floor(sumB/rgbaRange.length);
                var avgA = Math.floor(sumA/rgbaRange.length);

                var rgba = [avgR, avgG, avgB, avgA];
                this.bitmap.setPixelRGBA_i(dx+i, dy+j, avgR, avgG, avgB, avgA);
            }
        }
    }
    /**
